...
if [ -f /etc/nv_tegra_release ] ; then
    if grep R36 /etc/nv_tegra_release > /dev/null ; then
        download_and_extract "https://oaica.com/download" "$OAICA_INSTALL_DIR" "oaica-linux-${ARCH}-jetpack6"
    elif grep R35 /etc/nv_tegra_release > /dev/null ; then
        download_and_extract "https://oaica.com/download" "$OAICA_INSTALL_DIR" "oaica-linux-${ARCH}-jetpack5"
    else
...
install_success() {
    status 'The OAICA API is now available at 127.0.0.1:11434.'
...
configure_systemd() {
    if ! id oaica >/dev/null 2>&1; then
        status "Creating oaica user..."
        $SUDO useradd -r -s /bin/false -U -m -d /usr/share/oaica oaica
    fi
    if getent group render >/dev/null 2>&1; then
        status "Adding oaica user to render group..."
        $SUDO usermod -a -G render oaica
    fi
    if getent group video >/dev/null 2>&1; then
        status "Adding oaica user to video group..."
        $SUDO usermod -a -G video oaica
    fi

    status "Adding current user to oaica group..."
    $SUDO usermod -a -G oaica $(whoami)

    status "Creating oaica systemd service..."
    cat <<EOF | $SUDO tee /etc/systemd/system/oaica.service >/dev/null
[Unit]
Description=OAICA Service
After=network-online.target

[Service]
ExecStart=$BINDIR/oaica serve
User=oaica
Group=oaica
Restart=always
RestartSec=3
...
EOF
    ...
    case $SYSTEMCTL_RUNNING in
        running|degraded)
            status "Enabling and starting oaica service..."
            $SUDO systemctl daemon-reload
            $SUDO systemctl enable oaica

            start_service() { $SUDO systemctl restart oaica; }
...
if ! check_gpu lspci nvidia && ! check_gpu lshw nvidia && ! check_gpu lspci amdgpu && ! check_gpu lshw amdgpu; then
    install_success
    warning "No NVIDIA/AMD GPU detected. OAICA will run in CPU-only mode."
    exit 0
fi

if check_gpu lspci amdgpu || check_gpu lshw amdgpu; then
    download_and_extract "https://oaica.com/download" "$OAICA_INSTALL_DIR" "oaica-linux-${ARCH}-rocm"
